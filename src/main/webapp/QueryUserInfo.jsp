...
<center>
	<table class="table">
              ...
              <tbody>
    				...
    				<tr>
		    			...
		    			<td><%if(user.getUsername()==null){
    						%>未添加
    					<%}else{ %>    					
    					<%=user.getUsername() %>
    					<%}%></td>
		    			<td><%if(user.getSex()==null){
    						%>未添加
    					<%}else{ %>    					
    					<%=user.getSex() %>
    					<%}%></td>
		    			<td><%if(user.getAddress()==null){
    						%>未添加
    					<%}else{ %>    					
    					<%=user.getAddress() %>
    					<%}%></td>
		    			<td><%if(user.getPhone()==null){
    						%>未添加
    					<%}else{ %>    					
    					<%=user.getPhone() %>
    					<%}%></td>
		    			<td><%if(user.getPost()==null){
    						%>未添加
    					<%}else{ %>    					
    					<%=user.getPost() %>
    					<%}%></td>
		    			<td><%if(user.getEmail()==null){
    						%>未添加
    					<%}else{ %>    					
    					<%=user.getEmail() %>
    					<%}%></td>
		    			<td><%if(user.getRetime()==null){
    						%>未添加
    					<%}else{ %>    					
    					<%=user.getRetime() %>
    					<%}%></td>
		    			<td><%if(user.getLevel()==null){
    						%>未添加
    					<%}else{ %>    					
    					<%=user.getLevel() %>
    					<%}%></td>		    					    					    			
	...
	<a href='userInfo.jsp?id=${userlogin.id}'>添加或更改详细信息</a><br/><br/>
	<a href='updatevip.action?id=${userlogin.id}'>申请成为会员</a>	<br/><br/>
	<a href='UserfindOrderList.action?id=${userlogin.id}'>查看我的订单</a>	<br/><br/>
